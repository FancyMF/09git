...
static const CGFloat kaspect = 498.0f/660.0f ;//等比例缩放比例
...
//        }
        ...
        if (type == CropCornerTypeUpperLeft) {
            roundview.center=CGPointMake(0, 0 );
        }else if (type == CropCornerTypeUpperRight) {
             roundview.center=CGPointMake(CGRectGetMaxX(self.frame), 0 );
            } else if (type == CropCornerTypeLowerRight) {
                roundview.center=CGPointMake(CGRectGetMaxX(self.frame), CGRectGetMaxY(self.frame));
                   } else if (type == CropCornerTypeLowerLeft) {
                       roundview.center=CGPointMake(0, CGRectGetMaxY(self.frame));
                      }
    ...
    if ([touches count] == 1) {
        ...
        if (distanceBetweenPoints(location, p0) < kCropViewHotArea) {
            if (canChangeWidth) {
                ...
                frame.size.height=frame.size.width/kaspect;//自家等比例缩放，可去除。
            }
            if (canChangeHeight) {
                frame.origin.y += location.y;
                frame.size.height -= location.y;
                frame.size.width = frame.size.height * kaspect;//自家等比例缩放，可去除。
            }
...
//        CGRect bounds =(image.size.width>image.size.height)?CGRectMake(0, 0, image.size.height / scale *498/660, image.size.height / scale):CGRectMake(0, 0, image.size.height / scale , image.size.width / scale / 498*660);
...
//        _cropView = [[CropView alloc] initWithFrame:self.scrollView.frame];
        //201810
        float aspest = 498.0f/660.0f;
        CGRect cropBounds ;
        if(image.size.width <= image.size.height*aspest ){
            cropBounds=CGRectMake(0, 0, self.scrollView.frame.size.width, self.scrollView.frame.size.width/aspest);
        }
        else{
            cropBounds=CGRectMake(0, 0, self.scrollView.frame.size.height* aspest, self.scrollView.frame.size.height);
        }

        _cropView = [[CropView alloc] initWithFrame:cropBounds];
        _cropView.center = self.scrollView.center;
        _cropView.delegate = self;
        [self addSubview:_cropView];
        self.scrollView.frame=_cropView.frame;
...
{NSLog(@"orignx:%.0f,crop:%0.f",self.originalSize.width,cropView.bounds.size.width);
 NSLog(@"origny:%.0f,crop:%0.f",self.originalSize.height,cropView.bounds.size.height);
    ...
    NSLog(@"orignal : %0.f_____new:%0.f",contentOffset.x,newContentOffset.x);
